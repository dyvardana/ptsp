        ...
        <div className="border-t border-base-300 p-10 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-base-200 p-6 rounded-lg shadow">
              <h1 className="text-xl font-bold mb-4">Detail Tiket</h1>
              <p><strong>Nama Pemohon:</strong> {data.nama_pemohon}</p>
              <p><strong>Email:</strong> {data.email}</p>
              <p><strong>Karegori:</strong> {data.kategori_pengguna}</p>
            ...
            <div className="bg-base-200 p-6 rounded-lg shadow">
              ...
              <p><strong>Keterangan: {data.keterangan_tiket}</strong></p>
              <p><strong>Tanggal:</strong> 
              <span>
  {tindak_lanjut?.updated_at
    ? new Date(tindak_lanjut.updated_at).toLocaleString()
    : data?.updated_at
      ...
      : "-"
              ...
              <div className='w-full'>
                {data.status==="selesai"?(
                  ...
                  <li className="step step-error">
                    <span className="step-icon"><X size={18} /></span>
                    Ditolak
                  </li>
...
<form onSubmit={handleSubmit} className="space-y-3">
      <h2 className="text-base font-semibold">Penilaian</h2>
          ...
          <div className="rating rating-sm">
            {[1, 2, 3, 4, 5].map((star) => (
              <input
                ...
                checked={ratings[item.name] === star}
                onChange={() => handleRatingChange(item.name, star)}
              />
            ))}
          </div>
        </div>
      ...
      <button type="submit" className="btn btn-success btn-sm w-full">Kirim</button>
    </form>
          </>
        ):data.status==="selesai" && feedback?(
          <>
      ...
      {[
  { label: 'Kecepatan', name: 'kecepatan', value: feedback.kecepatan },
  { label: 'Kesesuaian', name: 'kesesuaian', value: feedback.kesesuaian },
  { label: 'Kemudahan', name: 'kemudahan', value: feedback.kemudahan }
].map((item) => (
  <div key={item.name} className="flex items-center justify-between">
    <span className="text-sm">{item.label}</span>
    <div className="rating rating-sm">
